...
const Contact = () => {
  ...
  const handleSend = (e) => {
    ...
    } else if (!emailValidation.apply(user_email)) {
    ...
    emailjs
      .sendForm(
      'service_5npadue', 'template_u4h2bbk', form.current, 'jNI6aJWaNVdqc78jU'
      )
      .then(
        (result) => {
            ...
            <form  ref={form} onSubmit={handleSend}className="w-full flex flex-col gap-4 lgl:gap-6 py-2 lgl:py-5">
              ...
              <div className="flex flex-col gap-4">
                ...
                <textarea
                  ...
                  } contactTextArea`}onFocus={handleFocus}
                  cols="30"
                  rows="8"
                ></textarea>
              </div>
              <div className="w-full">
                <button type="submit"
                  onClick={handleSend}
                  className="w-full h-12 bg-[#141518] rounded-lg text-base text-gray-500 tracking-wider 
                  uppercase hover:text-white duration-300 hover:border-[1px] hover:border-designColor border-transparent"
                >
                  Send Message
                </button>
              </div>
